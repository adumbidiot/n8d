var DisplaySWF = (function () {
	...
	function add_css() {
		...
		style.id = 'svelte-ic2vd8-style';
		style.textContent = ".nav.svelte-ic2vd8{width:100%;height:3rem;background-color:grey;position:static;float:right;display:inline}.nav-item.svelte-ic2vd8{height:2rem;left:0.5rem;top:0.5rem;position:relative}.nav-input.svelte-ic2vd8{width:12rem;border:0px;border-top-right-radius:1rem;border-top-left-radius:0rem;border-bottom-right-radius:1rem;border-bottom-left-radius:0rem;outline:none}.nav-button.svelte-ic2vd8{background-color:green;width:6rem;float:left;display:flex;align-items:center;justify-content:center;user-select:none;border-top-left-radius:1rem;border-bottom-left-radius:1rem}.nav-button.svelte-ic2vd8:active{background-color:#00FF00}";
	...
	function create_main_fragment(component, ctx) {
		...
		return {
			c() {
				...
				input.className = "nav-input nav-item svelte-ic2vd8";
				addListener(div_1, "click", click_handler);
				div_1.className = "nav-button nav-item svelte-ic2vd8";
				div.className = "nav svelte-ic2vd8";
			},
	...
	function Display_swf_nav(options) {
		...
		if (!document.getElementById("svelte-ic2vd8-style")) add_css();
	...
	/* src\display-swf-tag.html generated by Svelte v2.7.1 */



	function create_main_fragment$1(component, ctx) {
		var details, summary, text_value = ctx.tag.name, text, text_1, text_2_value = ctx.tag.recordHeader.code, text_2, text_3, text_4;

		function select_block_type(ctx) {
			if (getReadable(ctx.tag) != -1) return create_if_block;
		...
		return {
			...
			p(changed, ctx) {
				if ((changed.tag) && text_value !== (text_value = ctx.tag.name)) {
					text.data = text_value;
				}

				if ((changed.tag) && text_2_value !== (text_2_value = ctx.tag.recordHeader.code)) {
	...
	// (8:2) {#each Object.keys(tag) as key}
	function create_each_block(component, ctx) {
		var div, text_value = ctx.key, text, text_1, text_2_value = ctx.tag[ctx.key], text_2;
		...
		return {
			...
			p(changed, ctx) {
				if ((changed.Object || changed.tag) && text_value !== (text_value = ctx.key)) {
					text.data = text_value;
				}

				if ((changed.tag || changed.Object) && text_2_value !== (text_2_value = ctx.tag[ctx.key])) {
					text_2.data = text_2_value;
				}
			},
	...
	// (5:1) {#if getReadable(tag) != -1}
	function create_if_block(component, ctx) {
		var raw_value = getReadable(ctx.tag), raw_before, raw_after;
		...
		return {
			...
			p(changed, ctx) {
				if ((changed.tag) && raw_value !== (raw_value = getReadable(ctx.tag))) {
	...
	function create_if_block_1(component, ctx) {
		...
		var each_value = ctx.Object.keys(ctx.tag);
		...
		return {
			...
			p(changed, ctx) {
				if (changed.Object || changed.tag) {
					each_value = ctx.Object.keys(ctx.tag);
	...
	function get_each_context(ctx, list, i) {
		...
		child_ctx.key_index = i;
		return child_ctx;
	}

	function Display_swf_tag(options) {
		init(this, options);
	...
	assign(Display_swf_tag.prototype, proto);
	...
	function add_css$1() {
		var style = createElement("style");
		style.id = 'svelte-m17cri-style';
		style.textContent = ".main.svelte-m17cri{width:100%;height:100%;background-color:black;position:relative}.parse-info.svelte-m17cri{color:green;top:3rem;position:absolute;overflow-y:scroll;width:100%;bottom:0rem}";
		appendNode(style, document.head);
	}
	...
	function create_main_fragment$2(component, ctx) {
		...
		var displayswfnav_initial_data = { navURL: ctx.navURL };
		var displayswfnav = new Display_swf_nav({
			root: component.root,
			data: displayswfnav_initial_data
		});
		...
		var if_block = (ctx.tags.length > 0) && create_if_block$1(component, ctx);
		...
		return {
			c() {
				...
				div_1.className = "main svelte-m17cri";
			},
			...
			p(changed, ctx) {
				var displayswfnav_changes = {};
				if (changed.navURL) displayswfnav_changes.navURL = ctx.navURL;
				displayswfnav._set(displayswfnav_changes);
	...
	// (41:4) {#each tags as tag}
	function create_each_block$1(component, ctx) {

		var displayswftag_initial_data = { tag: ctx.tag };
		var displayswftag = new Display_swf_tag({
			root: component.root,
			data: displayswftag_initial_data
		});

		return {
			c() {
				displayswftag._fragment.c();
			},

			m(target, anchor) {
				displayswftag._mount(target, anchor);
			},

			p(changed, ctx) {
				var displayswftag_changes = {};
				if (changed.tags) displayswftag_changes.tag = ctx.tag;
				displayswftag._set(displayswftag_changes);
			},

			d(detach) {
				displayswftag.destroy(detach);
			}
		};
	}

	// (20:2) {#if tags.length > 0}
	function create_if_block$1(component, ctx) {
		var div, div_1, text_2, div_2, text_3, text_4_value = ctx.SWFTools.VERSION, text_4, text_5, div_3, text_6, text_7_value = ctx.SWFTools.SWFParser.VERSION, text_7, text_8, div_4, text_9, text_10, text_11, div_5, text_12, text_13, text_14, div_6, text_15, text_16_value = ctx.startTime.getTime(), text_16, text_17, text_18, div_7, text_19, text_20_value = ctx.endTime.getTime(), text_20, text_21, text_22, div_8, text_23, text_24, text_25, text_26, div_9, text_27, text_28, text_29, text_30, div_10, text_31, text_32_value = ctx.tags.length, text_32, text_33, text_34, div_11, text_37, br, text_38, div_12, text_41, text_42, div_13;
		...
		return {
			c() {
				...
				div.className = "parse-info svelte-m17cri";
			},
			...
			p(changed, ctx) {
				...
				if ((changed.tags) && text_32_value !== (text_32_value = ctx.tags.length)) {
	...
	function Display_swf(options) {
		...
		if (!document.getElementById("svelte-m17cri-style")) add_css$1();
